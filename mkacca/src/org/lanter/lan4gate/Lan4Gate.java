...
public class Lan4Gate implements ICommunicationListener {
	 ...
	 *
	 * @param ecrNumber Logical ECR number, used for communication with terminal.
	 *                  Value in range [1,999]
	 */
	 ...
	 */
	public int getPort() {
		return mTCPCommunication.getPort();
	}

	/**
	 * Sets IP for connection in TCP Client mode Currently is STUB
	 * 
	...
	/**
	 * Start protocol communication. For TCP Server mode (default state) it starts
	 * thread for {@link java.nio.channels.Selector} After correct start
	 * {@link ICommunicationCallback}.communicationStarted() will be called
	 */
	public void start() {
	...
	/**
	 * Returns state of link opening. For TCP Server mode it`s returns status of
	 * server socket opening. For TCP Client mode it`s returns status of client link
	 * opening
	 *
	...
	/**
	 * Returns state of link connection. For TCP Server mode it`s returns status
	 * remote client connection. For TCP Client mode it`s returns connection status
	 * to remote host.
	 * 
	...
	/**
	 * Stop protocol communication. After correct start
	 * {@link ICommunicationCallback}.communicationStopped() will be called
	 */
	public void stop() {
	...
	/**
	 * Prepare and returns {@link IRequest} object. Automatically fill ecrNumber,
	 * Type and mandatory fields
	 *
	...
	public void sendRequest(IRequest request) {
		...
		try {
			...
			if (result) {
				Log.d("LanterPOS","Request: "+assembler.getJson());
	...
	public void newData(String data) {
		...
		try {
			if (parser.parse(data)) {
				Log.d("LanterPOS","Parser OK, type "+parser.getType());
				if (parser.getType() == ClassFieldValuesList.Response) {
					...
					for (IResponseCallback callback : mResponseListeners) {
						Log.d("LanterPOS","Call new message");
