...
public class AppCore extends Application implements ServiceConnection {
    ...
    public static abstract class TaskWithDialog<I, R>  {
        ...
        abstract R doInBackground(@SuppressWarnings("unchecked") I...args);

        public TaskWithDialog<I, R> execute(@SuppressWarnings("unchecked") final I... args) {
        ...
        protected final void publishProgress(final String... values) {
            mHandler.post(new Runnable() {
				@Override
				public void run() {
     ...
     *
     * @return успешна ли инициализация
     */
    ...
    public void sendMessage(int msg, int arg, Object payload) {
        mQueue.sendMessage(msg, arg, payload);
    }
    ...
    public void onServiceConnected(ComponentName arg0, IBinder binder) {
            ...
            new FNOperaionTask(this, MSG_FISCAL_STORAGE_READY) {
                ...
                protected Integer doInBackground(Object... args) {
                    try {
                        R = mStorage.waitFnReady(mWaitFNTimeoutMs);
                        synchronized (mStorageLockObject) {
                            mStorageLockObject.notifyAll();
                        }

                    } catch (Exception e) {
                        mLogger.log(Level.SEVERE, "Ошибка", e);
        ...
        return new FNOperaionTask(context, 0) {
            ...
            protected Integer doInBackground(Object... args) {
                if(mStorage == null) {
                    ...
                    try {
                        synchronized (mStorageLockObject) {
                            while ((mStorage == null || (mStorage.waitFnReady(0) != Errors.NO_ERROR))
                                    && !mKillAll) {
	...
	public void schedule(Runnable runnable, long when) {
		mQueue.postDelayed(runnable,when);
		
	}
	public void invokeOnUI(Runnable r) {
		mQueue.post(r);
	}
