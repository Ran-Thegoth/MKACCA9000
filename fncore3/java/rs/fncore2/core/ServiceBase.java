...
class ServiceBase extends Service {
    ...
    protected int doArchive(OU operator, ArchiveReport report, String template) {
        if (!mFNManager.isFNReady()) return Errors.DEVICE_ABSEND;
...
/*    protected void printFnCounters(boolean total){
        if (!total && mFNManager.getFN().getKKMInfo().getFFDProtocolVersion().bVal< KKMInfo.FFDVersionE.VER_12.bVal) {
            Logger.i("Print XReport: " + mXReport);
            doPrint(mXReport.getPF(null));
            return;
        }

        FnCountersExBase fnCounters=mFNManager.getFN().getFnCounters(total);
        if (fnCounters!=null) {
            doPrint(fnCounters.getPF(null));
        }
    } */
...
/*            if (wasActiveBefore && reason!=KKMInfo.FiscalReasonE.REPLACE_FN) {
    ...
    protected int doCorrection2(Correction cor, OU operator, Correction signed, String header, String item,
        ...
        FNBaseI.doCorrectionResult res = mFNManager.getFN().doCorrection2(cor, operator, signed, header,
                item, footer, footerEx);
        if (res.code == Errors.NO_ERROR) {
            doPrint(res.print);
        }
        return res.code;
    ...
    protected int doSellOrder(SellOrder order, OU operator,
                              SellOrder signed, boolean is_doPrint, String header, String item,
                              String footer, String footerEx) {
        final long DAY = 24 * 60 * 60000L;

        if (!mFNManager.isFNReady()) return Errors.DEVICE_ABSEND;
        if (!mFNManager.getFN().getKKMInfo().getShift().isOpen()) return Errors.INVALID_SHIFT_STATE;

        long openShiftTime =
                System.currentTimeMillis() - mFNManager.getFN().getKKMInfo().getShift().getWhenOpen();
        if (openShiftTime > DAY) {
            Logger.e("Shift opened too much time: %s", convertTimeWithTimeZome(mFNManager.getFN().getKKMInfo().getShift().getWhenOpen()));
        ...
        if (res.code == Errors.NO_ERROR) {
            if (is_doPrint) doPrint(res.print);
        }
        return res.code;
    }
    ...
    protected void setOFDSettings(DocServerSettings s) {
        if (s == null) return;
        try {
            mSettings.setOFDServer(Utils.deserialize(Utils.serialize(s),
                    DocServerSettings.CREATOR));
        } catch (Exception e) {
    ...
    protected void setOismSettings(DocServerSettings s) {
        if (s == null) return;
        try {
            mSettings.setOISMServer(Utils.deserialize(Utils.serialize(s),
                    DocServerSettings.CREATOR));
        } catch (Exception e) {
    ...
    protected void setPrintSettings(PrintSettings s) {
        if (s == null) return;
        try {
            mSettings.setPrintSettings(Utils.deserialize(Utils.serialize(s),
                    PrintSettings.CREATOR));
        } catch (Exception e) {
    ...
    protected int exchangeB(byte[] inData, byte[] outData, long waitTimeoutMs) throws InterruptedException {
        ...
        if (transaction == 0) return Errors.DEVICE_ABSEND;

        try {
            int res = mFNManager.getFN().writeB(transaction, inData, 0, inData.length);
            if (res != inData.length) return Errors.DEVICE_ABSEND;
    ...
    int openShift(OU operator, Shift shift, String template) {
        ...
        if (res.code == Errors.NO_ERROR) {
            if (res.alreadyDone) return res.code;
    ...
    protected int putOrWithdrawCash(double value, OU operator, String template) throws RemoteException {
        if (Math.abs(value) < 0.01) return Errors.SUM_MISMATCH;
	...
	public int getFNCounters(FNCounters doc, boolean isShiftCounters) {
		...
		try(Transaction transaction = mFNManager.getFN().getStorage().open()) {
			...
			if(result == null) return Errors.DATA_ERROR;
