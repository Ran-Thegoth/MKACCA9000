...
public class FNSNAccess {
    ...
    public FNSNAccess(Context context) {
        ...
        try {
            ...
            int read;
            while ((read = is.read(buff)) > 0) fs.write(buff, 0, read);
            fs.close();
            is.close();
            ...
            for (int i = 0; i < l; i++) {
                if (NUMBERS.indexOf((char) b[i]) == -1) break;
                mFNSN += (char) b[i];
            }

            if (mFNSN.isEmpty()) {
                String s = rs.fncore.UrovoUtils.getProp("ro.serialno");
                if (s == null || s.isEmpty()) s = Build.SERIAL;
                Log.e("fncore2", String.format("ro.serialno %s", s));
